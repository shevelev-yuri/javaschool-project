...
<c:set var="dateFilter" value="today"/>
...
<c:if test="${pages.totalPages > 1}">
    <ul class="pagination">
        <c:choose>
            ...
            <c:otherwise>
                <li class="page-item">
                   <a class="page-link" href="eventstoday?page=${pages.currentPage - 1}">Previous</a>
                </li>
            </c:otherwise>
        </c:choose>
        <c:forEach items="${pages.pages}" var="page">
            <c:if test="${page != -1}">
                <c:if test="${pages.currentPage != page}">
                    <li class="page-item">
                        <a class="page-link" href="eventstoday?page=${page}">${page}</a>
                    </li>
                </c:if>
                <c:if test="${pages.currentPage == page}">
        ...
        <c:choose>
            ...
            <c:otherwise>
                <li class="page-item">
                    <a class="page-link" href="eventstoday?page=${pages.currentPage + 1}">Next</a>
                </li>
            </c:otherwise>
        </c:choose>
    </ul>
